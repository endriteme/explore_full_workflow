...
echo "Assigning default 45-day trial license..."

TRIAL_LICENSE_RESPONSE="$(
  jq -n '{}' |
  curl --fail-with-body -sS -X POST \
...
TRIAL_DURATION="$(echo "$TRIAL_LICENSE_RESPONSE" | jq -r '.duration // "unknown"')"
echo "Trial license assigned successfully. Duration: ${TRIAL_DURATION} days"
