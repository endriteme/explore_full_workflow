...
if [[ -z "$INTAKE_KEY" ]]; then
  read -r -s -p "Enter your Intake Key: " INTAKE_KEY
  echo
fi
...
echo "Starting Windows event simulator. Press Ctrl+C to stop."
...
while true; do
  ...
  http_status=$(curl -sS -o /dev/null -w "%{http_code}" -X POST "${INTAKE_URL}" \
